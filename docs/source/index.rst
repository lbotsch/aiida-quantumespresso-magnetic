.. figure:: images/AiiDA_transparent_logo.png
    :width: 250px
    :align: center

The aiida-quantumespresso-magnetic plugin for `AiiDA`_
======================================================

``aiida-quantumespresso-magnetic`` is available at http://github.com/lbotsch/aiida-quantumespresso-magnetic


.. toctree::
   :maxdepth: 2

   user_guide/index
   developer_guide/index
   API documentation <apidoc/aiida_quantumespresso_magnetic>

If you use this plugin for your research, please cite the following work:

.. highlights:: Author Name1, Author Name2, *Paper title*, Jornal Name XXX, YYYY (Year).

If you use AiiDA for your research, please cite the following work:

.. highlights:: Giovanni Pizzi, Andrea Cepellotti, Riccardo Sabatini, Nicola Marzari,
  and Boris Kozinsky, *AiiDA: automated interactive infrastructure and database
  for computational science*, Comp. Mat. Sci 111, 218-230 (2016);
  https://doi.org/10.1016/j.commatsci.2015.09.013; http://www.aiida.net.

``aiida-quantumespresso-magnetic`` is released under the MIT license. 

Please contact lukas.botsch@gmail.com for information concerning ``aiida-quantumespresso-magnetic`` and the `AiiDA mailing list <http://www.aiida.net/mailing-list/>`_ for questions concerning ``aiida``.


Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`

.. _AiiDA: http://www.aiida.net

